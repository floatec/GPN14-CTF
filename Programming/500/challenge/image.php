...
$grey[] = imagecolorallocate($img, rand(0,255), rand(0,255), rand(0,255));
$type = rand(0,9);
$backimg = imagecreatefrompng('./data/'.$type.'/back.png');

imagecopy($img, $backimg, 0,0,0,0,175,50);
...
for ($i =0; $i < strlen($text); $i++)
{
    ...
    imagettftext($img, $size, $angle, $off, 40,$grey[$i], $font , $text[$i]);
}
...
#imagestring($img, 1 , 0, $imgHeight-10, ($time*1000)."ms to create" , 0xffffff);
ImagePNG($img);
imagedestroy($img);
